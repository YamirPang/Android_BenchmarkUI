...
public class ARCLoadingView extends View {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        if (null != attrs) {
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.ArcLoadingView);
            mArcColor = typedArray.getColor(R.styleable.ArcLoadingView_lv_color, DEFAULT_ARC_COLOR);
            mArcDegree = typedArray.getInt(R.styleable.ArcLoadingView_lv_arc_degree, DEFAULT_ARC_DEGREE);
            if (mArcColor != DEFAULT_ARC_COLOR) {
                mGradientColors = new int[]{Color.TRANSPARENT, mArcColor};
            }
            mArcWidth = typedArray.getDimensionPixelSize(R.styleable.ArcLoadingView_lv_width, DensityUtils.dp2px(getContext(), DEFAULT_ARC_WIDTH));
            mSpeedOfDegree = typedArray.getInt(R.styleable.ArcLoadingView_lv_speed, DEFAULT_SPEED_OF_DEGREE);
            mIsDraw = mIsAutoMode = typedArray.getBoolean(R.styleable.ArcLoadingView_lv_auto, true);

            boolean hasIcon = typedArray.getBoolean(R.styleable.ArcLoadingView_lv_has_icon, true);
            if (hasIcon) {
                Drawable icon = typedArray.getDrawable(R.styleable.ArcLoadingView_lv_icon);
                if (icon != null) {
                    mIconBitmap = CommonUtils.getBitmapFromDrawable(icon);
                } else {
                    Drawable appIcon = UIConfig.getInstance().getAppIcon();
                    if (appIcon != null) {
                        mIconBitmap = CommonUtils.getBitmapFromDrawable(appIcon);
                    }
                }
                mIconScale = typedArray.getFloat(R.styleable.ArcLoadingView_lv_icon_scale, 0.5F);
            }
            typedArray.recycle();
        }
